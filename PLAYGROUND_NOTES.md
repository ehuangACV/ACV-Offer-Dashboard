...
## 2026-09-25 加一道"软性"密码门槛(GitHub Pages 上公开访问用)

这个 Playground 也发到 GitHub Pages 上给别人看,你要求加个密码
(`ACVux2026!`),验证通过后"记住"这台设备/浏览器,下次打开不用再输。

跟你说明过、你确认接受的前提:这只能是**软性**保护,不是真正的安全
防护——纯静态站点没有后端,密码校验只能写在页面自己的 JS 里,任何人开
开发者工具看源码都能直接看到密码,或者绕过检查强行看到 `#app`。适合
"拦一下随手点进来的人",不适合保护真正敏感数据(这个 Playground 本身
只有 mock 数据,没有这个问题)。

实现:`#app` 默认 `display:none`,`<body>` 里紧跟着加一个
`#pg-lock-screen` 全屏遮罩(密码输入框+提交按钮+错误提示),和一段
放在 Vue 的 `<script src>` **之前**的纯 vanilla JS(不依赖 Vue,故意
不等 Vue CDN 加载/mount 完成才判断,避免"密码遮罩还没盖上就先露了一下
内容"的闪烁)。逻辑:页面加载时检查 `localStorage['pg_unlocked_v1']`
是否为 `'yes'`——是就直接隐藏遮罩、显示 `#app`;不是就显示遮罩,监听
表单提交,密码对了就写这条 localStorage 标记再解锁,不对就在原地显示
"密码不对,再试一次"并清空输入框。"记住"选的是 localStorage(不是
sessionStorage)——这台浏览器以后打开都不用再输,除非清了浏览器数据或
换了别的设备/浏览器。
