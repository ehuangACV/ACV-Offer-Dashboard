...
## 2026-09-10（第三次）隐藏原生横向滚动条，避免和影子滚动条重复出现

你反馈滚到某个位置时最底部会同时出现两条横向滚动条——上面细的是影子
滚动条，下面粗的是 `table-scroll` 自己原生的横向滚动条（一直都在，
没有被隐藏过，只是之前窄屏/没滚到底时刚好看不到，滚到刚好能同时露出
两者的位置就叠在一起了）。

现在横向滚动的交互已经全部由影子滚动条承担，原生那条完全是多余的。
给 `.offer-dashboard__table-scroll` 加了 `scrollbar-width:none`
(Firefox) + `::-webkit-scrollbar{display:none}`(Chrome/Safari/Edge)
——只隐藏原生滚动条的**外观**，横向滚动本身的功能（`scrollLeft`/
wheel）不受影响，影子滚动条本来就是靠同步 `scrollLeft` 实现的，不
依赖这条原生滚动条的视觉表现。
